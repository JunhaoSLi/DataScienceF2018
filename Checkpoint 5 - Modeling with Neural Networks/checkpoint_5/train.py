...
from analysis import checkpoint5 as checkpoint5_analysis
from datasets import checkpoint5
...
def transform_checkpoint5(statements):
    n_batch = len(statements)
    xmb = np.zeros((n_batch, n_ctx, 2), dtype=np.int32)
    mmb = np.zeros((n_batch, n_ctx), dtype=np.float32)
    start = encoder['_start_']
    for i, statement in enumerate(statements):
        if not isinstance(statement[0], int):
            statement = sum(statement, [])
        full = [start] + statement[:max_len] + [clf_token]
        length = len(full)
        xmb[i, :length, 0] = full
        mmb[i, :length] = 1
...
if __name__ == '__main__':
    ...
    vaX, vaM = transform_checkpoint5(vaX1)
    if submit:
        teX, teM = transform_checkpoint5(teX1)
    ...
    if submit:
        ...
        if args.analysis:
            checkpoint5_analysis(data_dir, os.path.join(args.submission_dir, 'Checkpoint5.tsv'),
                                os.path.join(log_dir, 'checkpoint5.jsonl'))
