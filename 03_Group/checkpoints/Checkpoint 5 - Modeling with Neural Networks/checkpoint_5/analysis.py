...
from datasets import _checkpoint5

def checkpoint5(data_dir, pred_path, log_path):
    preds = pd.read_csv(pred_path, delimiter='\t')['prediction'].values.tolist()
    _, labels = _checkpoint5(os.path.join(data_dir, 'testing.csv'))
    ...
    print('Checkpoint 5 Valid Accuracy: %.2f'%(valid_accuracy))
    print('Checkpoint 5 Test Accuracy:  %.2f'%(test_accuracy))
